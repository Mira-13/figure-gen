...
images = [
    manage_images.load_image(scene='pool', method=method_name) for method_name in method_names
]
colors = [
    [232, 181, 88],
    [5, 142, 78],
...
errors = [
    manage_images.get_error(scene='pool', method=method) for method in method_names
]
...
def method_caption(name, image, error, rgb_list):
    ...
    if name == names[-1]: # if Ours then highlight
        ...
        title = "\\textsf{"+name+"}"
        if name == names[0]:
            speedup = "\\textsf{("+ f"{errors[baseline] * 1./error:.2f}" + "x)}"
        else: 
...
captions.append(r"\vspace*{5.0pt}\textsf{Reference}" + "\n" + r"\textsf{\emph{relMSE} ("+ str(manage_images.get_time_sec(scene='pool')) +"s)}")
